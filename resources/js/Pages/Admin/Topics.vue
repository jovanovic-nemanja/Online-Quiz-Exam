<template>
    <admin-layout>
        <template #header>
            <h4 class="page-heading">Topics</h4>
        </template>
        <template #actions>
            ...
            <div class="card">
                <div class="card-body">
                    <vue-good-table mode="remote" @on-page-change="onPageChange" @on-column-filter="onColumnFilter" @on-per-page-change="onPerPageChange"
                                    :pagination-options="options" :columns="columns" :totalRows="topics.meta.pagination.total" :rows="topics.data">
                        <template slot="table-row" slot-scope="props">
                            ...
                            <div v-else-if="props.column.field === 'actions'">
                                ...
                                <Button icon="pi pi-trash" class="p-button-rounded p-button-danger p-button-text"
                                        @click="deleteTopic(props.row.id)"/>
                            </div>
                            <!-- Remaining Columns -->
                            <span v-else>
                    ...
                    <Sidebar position="right" :visible.sync="createForm" v-on:hide="loadItems" class="p-sidebar-md">
                        <TopicForm :skills="skills" :errors="errors" @close="createForm = false" title="New Topic"/>
                    </Sidebar>
                    <Sidebar position="right" :visible.sync="editForm" v-on:hide="loadItems" class="p-sidebar-md">
                        <TopicForm :skills="skills" :edit-flag.sync="editForm" :topic-id="currentId" :errors="errors"
                                   @close="editForm = false" title="Edit Topic"/>
                    </Sidebar>
...
<script>
    ...
    export default {
        ...
        computed: {
            title() {
                return 'Topics - ' + this.$page.props.general.app_name + ' Admin';
            }
        },
        methods: {
