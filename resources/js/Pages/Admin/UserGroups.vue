<template>
    <admin-layout>
        <template #header>
            <h4 class="page-heading">User Groups</h4>
        </template>
        <template #actions>
            ...
            <div class="card">
                <div class="card-body">
                        ...
                        <template slot="table-row" slot-scope="props">
                            ...
                            <div v-else-if="props.column.field === 'actions'">
                                <Button icon="pi pi-pencil" class="p-button-rounded p-button-secondary p-button-text p-mr-2"
                                 ...
                                 <Button icon="pi pi-trash" class="p-button-rounded p-button-danger p-button-text" @click="deleteUserGroup(props.row.id)"/>
                            </div>
                    ...
                    <Sidebar position="right" :visible.sync="createForm" v-on:hide="loadItems" class="p-sidebar-md">
                        <UserGroupForm :errors="errors" @close="createForm = false" title="New User Group"/>
                    </Sidebar>
                    <Sidebar position="right" :visible.sync="editForm" v-on:hide="loadItems" class="p-sidebar-md">
                        <UserGroupForm :edit-flag.sync="editForm" :user-group-id="currentId" :errors="errors"
                                       @close="editForm = false" title="Edit User Group"/>
                    </Sidebar>
...
<script>
    ...
    export default {
        ...
        computed: {
            title() {
                return 'User Groups - ' + this.$page.props.general.app_name + ' Admin';
            }
        },
        methods: {
