        ...
        <div v-show="!collapse" :id="question.code+'_options'">
            <ul class="q-option" v-for="(answer, index) in question.correct_answer">
                <li class="flex items-center mb-3 cursor-pointer p-3 rounded-sm border border-gray-200">
                    ...
                    <div class="text-sm font-normal ml-4" v-html="answer"></div>
                </li>
            </ul>
        </div>
        <a @click="collapse = !collapse" class="qt-link-success text-sm mb-4" v-html="collapse ? 'View Answers' : 'Hide Answers'"></a>
        <ul class="mt-5">
            <li class="text-gray-600 text-sm flex items-center text-sm mb-4">
                <span class="font-semibold">Question Type:</span>
                <span class="text-sm ml-2">{{ question.question_type_name }}</span>
            </li>
            <li class="text-gray-600 text-sm flex items-center text-sm mb-4">
                <span class="font-semibold">Difficulty Level:</span>
                <span class="text-sm ml-2">{{ question.difficulty }}</span>
            </li>
            <li class="text-gray-600 text-sm flex items-center text-sm mb-4">
                <span class="font-semibold">Marks/Points:</span>
                <span class="text-sm ml-2">{{ question.marks }} XP</span>
            </li>
            <li class="text-gray-600 text-sm flex items-center text-sm mb-4">
