<template>
    <admin-layout>
        <template #header>
            <h4 class="page-heading">Comprehensions</h4>
        </template>
        <template #actions>
            ...
            <div class="card">
                <div class="card-body">
                    <vue-good-table mode="remote" @on-page-change="onPageChange" @on-column-filter="onColumnFilter" @on-per-page-change="onPerPageChange"
                                    :pagination-options="options" :columns="columns" :totalRows="comprehensions.meta.pagination.total" :rows="comprehensions.data">
                    ...
                    <Sidebar position="right" :visible.sync="createForm" v-on:hide="loadItems" class="p-sidebar-md">
                        <ComprehensionForm :errors="errors" @close="createForm = false" title="New Comprehension"/>
                    </Sidebar>
                    <Sidebar position="right" :visible.sync="editForm" v-on:hide="loadItems" class="p-sidebar-md">
                        <ComprehensionForm :edit-flag.sync="editForm" :comprehension-id="currentId" :errors="errors"
                                      @close="editForm = false" title="Edit Comprehension"/>
                    </Sidebar>
                </div>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        computed: {
            title() {
                return 'Comprehensions - ' + this.$page.props.general.app_name + ' Admin';
            }
        },
        methods: {
